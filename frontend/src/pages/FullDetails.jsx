...
export default function FullDetails() {
  ...
  // Helper: normalize show.movie (ObjectId or populated) to string
  ...
  useEffect(() => {
    ...
    const fetchMovie = async () => {
      try {
        ...
        if (!cancelled) {
          setMovie(res.data.movie ?? res.data ?? null);
        }
      } catch (err) {
    ...
    // Fetch shows, then filter by movie id + isActive + optional future date
    const fetchShows = async () => {
      try {
        ...
        const filtered = allShows
          ...
          // only active shows
          .filter((s) => s.isActive)
          // OPTIONAL: only future or today shows
          // .filter((s) => new Date(s.date) >= today)
          // sort by date ascending
          .sort((a, b) => new Date(a.date) - new Date(b.date));

        console.log("Filtered shows for movie", id, filtered);
        if (!cancelled) {
          setShows(filtered);
        }
      } catch (err) {
  ...
  if (loading) {
    return (
      <div className="text-center py-10 text-gray-600">Loading details…</div>
    );
  }
  if (error) {
    return (
...
<<<<<<< HEAD
          ...
          <p className="text-primary">
            Languages: {movie.language?.toUpperCase() || "N/A"}
          </p>
...
<<<<<<< HEAD
      ...
      {/* You May Also Like */}
      <p className="text-lg font-medium mt-20 mb-10">
        You May Also Like
      </p>
